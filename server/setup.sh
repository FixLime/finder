...
apt install -y ufw certbot python3-certbot-nginx git build-essential
...
ufw allow 3478/tcp      # TURN server
...
// Config
const JWT_SECRET = 'finder_jwt_secret_2024_' + uuidv4().substring(0, 8);
const PORT = 3000;
const WS_PORT = 3001;
...
app.put('/api/calls/:callId', auth, async (req, res) => {
    const { status } = req.body; // 'accepted', 'rejected', 'ended'
